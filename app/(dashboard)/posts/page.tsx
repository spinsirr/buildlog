      ...
      <CardContent className="pt-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {/* Regenerate — only for AI-generated drafts */}
  ...
  return (
    <div className="flex flex-col gap-3">
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className="bg-zinc-900 border border-zinc-800 rounded-lg p-5 space-y-4 animate-pulse"
        >
          <div className="h-4 bg-zinc-800 rounded w-3/4" />
          <div className="h-4 bg-zinc-800 rounded w-1/2" />
          <div className="flex gap-2">
            <div className="h-5 bg-zinc-800 rounded-full w-10" />
...
export default function PostsPage() {
  ...
  async function handleRegenerate(id: string) {
    ...
    mutate(
      (current) => {
        if (!current) return current;
        return {
          posts: current.posts.map((p) => (p.id === id ? { ...p, ...post } : p)),
        };
      },
      { revalidate: false }
    );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {error ? (
        <div className="text-sm text-red-400">Failed to load posts.</div>
      ) : isLoading ? (
        <PostsSkeleton />
      ) : (
